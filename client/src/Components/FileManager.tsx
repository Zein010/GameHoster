...
export default function FileManager() {
    ...
    useEffect(() => {
        // add or remove refs
        setFileRowsRef((fileRows) => files.map((_, i) => fileRows[i] || createRef()));
    }, [files]);
    useEffect(() => {}, [fileRowsRef]);
    useEffect(() => {
    ...
    const handleUploadFile = (event: any) => {
        ...
        axios
            .post(import.meta.env.VITE_API + `/Files/${id}/Upload`, formData, {
                headers: {
    ...
    return (
        <Box>
                ...
                <CardContent>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "start", gap: 1 }}>
                            ...
                            {currentPath != "" ? (
                                <Button
                                    ...
                                    onClick={() => {
                                        ChangePath(currentPath.substring(0, currentPath.lastIndexOf("/")));
                                    }}
                                >
                                    {" "}
                                    Back
                    ...
                    <Stack spacing={1} sx={{ textAlign: "left" }}>
                        ...
                        {files.map((file, i) => (
                            <Stack
                                onDoubleClick={(e) => {
                                    file.isDirectory ? ChangePath((currentPath != "" ? currentPath + "/" : "") + file.name) : file.textEditable ? EditFile((currentPath != "" ? currentPath + "/" : "") + file.name) : "";
                                    e.preventDefault();
                                }}
                                onClick={() => {
                                    fileRowClicked(i);
                                ...
                                ref={fileRowsRef[i]}
                                key={"file-" + i}
                                sx={{ flexGrow: 1, userSelect: "none", backgroundColor: clickedRowIndex == i ? "background.surface" : selectedFiles.includes(file.name) ? "background.surface" : "background.level2", "&:hover": { backgroundColor: "background.level3" } }}
                            >
                                <Item sx={{ flexBasis: "40px", borderEndEndRadius: 0, borderStartEndRadius: 0 }}>
                                    <Checkbox
                                        ...
                                        onChange={(e) => {
                                            fileCheckBoxClicked(file.name, e.target.checked);
                                        }}
                                    />
                                </Item>
                                <Item sx={{ flexGrow: 1, borderRadius: 0 }}>
                                    <Typography level="body-sm" sx={{ textAlign: "left", fontWeight: "bold", userSelect: "none" }}>
                                        {file.isDirectory ? <FolderIcon sx={{ mr: 1 }} color="primary" /> : <InsertDriveFileIcon color="warning" sx={{ mr: 1 }} />}
                                        {file.name}
                                    </Typography>
                                </Item>
                                <Item sx={{ flexBasis: "80px", borderRadius: 0 }}>
                                ...
                                <Item sx={{ flexBasis: "200px", borderRadius: 0 }}>
                                    <Typography level="body-sm" sx={{ textAlign: "left", color: "gray", userSelect: "none" }}>
                                        {file.modifiedAt.split(".")[0].replace("T", " ")}
                                    </Typography>
                                </Item>
                                <Item sx={{ flexBasis: "200px", borderEndStartRadius: 0, borderStartStartRadius: 0 }}>
                                    <Typography level="body-sm" sx={{ textAlign: "left", color: "gray", userSelect: "none" }}>
                                        {file.createdAt.split(".")[0].replace("T", " ")}
                                    </Typography>
                                </Item>
                            </Stack>
                        ))}
            ...
            <Modal aria-labelledby="modal-title" aria-describedby="modal-desc" open={confirmOpen} onClose={() => setConfirmOpen(false)} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <Sheet variant="outlined" sx={{ maxWidth: 500, borderRadius: "md", p: 3, boxShadow: "lg" }}>
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1, mt: 2 }}>
                        ...
                        <Button size="sm" onClick={() => setConfirmOpen(false)} color="neutral">
                            Cancel
                        </Button>
                    </Box>
                </Sheet>
            </Modal>
            <Modal aria-labelledby="modal-title" aria-describedby="modal-desc" open={createOpen} onClose={() => setCreateOpen(false)} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <Sheet variant="outlined" sx={{ maxWidth: 500, borderRadius: "md", p: 3, boxShadow: "lg" }}>
                    <ModalClose variant="plain" sx={{ m: 1 }} />
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1, mt: 2 }}>
                        <Button color="success" onClick={() => sendCreate()} size="sm" sx={{ mr: 2 }} startDecorator={createType == "file" ? <NoteAddIcon /> : <CreateNewFolderIcon />}>
                            {" "}
                            Create
                        </Button>
                        <Button size="sm" onClick={() => setCreateOpen(false)} color="neutral">
                            Cancel
                        </Button>
                    </Box>
                </Sheet>
            ...
            <Modal aria-labelledby="modal-title" aria-describedby="modal-desc" open={uploadOpen} onClose={() => setUploadOpen(false)} sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
                <Sheet variant="outlined" sx={{ maxWidth: 500, borderRadius: "md", p: 3, boxShadow: "lg" }}>
                    ...
                    <input
                        ...
                        multiple
                        onChange={(e) => {
                            e.target.files ? setUploadFiles(Array.from(e.target.files)) : null;
                        }}
                    />
                    <Box sx={{ display: "flex", justifyContent: "space-between", gap: 1, mt: 2 }}>
                        <Button color="success" onClick={(e) => handleUploadFile(e)} size="sm" sx={{ mr: 2 }} startDecorator={<UploadFile />}>
                            {" "}
                            Upload
                        </Button>
                        <Button size="sm" onClick={() => setUploadOpen(false)} color="neutral">
                            Cancel
                        </Button>
                    </Box>
                </Sheet>
