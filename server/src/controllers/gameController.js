...
const CheckServerRunning = async (req, res) => {
    ...
    const status = TerminalService.CheckUserHasProcess(server.sysUser.username, server.gameVersion.searchScript, (pid) => { GameService.SetRunningServerPID(serverDetails.id, pid) });
    res.json({ status });
}
