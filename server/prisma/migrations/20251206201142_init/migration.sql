...
-- AlterTable
ALTER TABLE `gameversion` ADD COLUMN `requiredCpu` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `requiredRam` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `requiredStorage` INTEGER NOT NULL DEFAULT 0;

-- AlterTable
ALTER TABLE `server` ADD COLUMN `cpu` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `priority` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `ram` INTEGER NOT NULL DEFAULT 0,
...
-- AddForeignKey
ALTER TABLE `ServerQueue` ADD CONSTRAINT `ServerQueue_serverId_fkey` FOREIGN KEY (`serverId`) REFERENCES `RunningServers`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
